...
impl Cache {
    pub fn new(data_dir: PathBuf) -> Result<Self, BasescopeError> {
    ...
    pub fn load_chunk(&self, start_block: u64, end_block: u64) -> Result<ChunkData, BasescopeError> {
        ...
        let col: ColumnarChunkData = bincode::deserialize(&decompressed[V1_MAGIC.len()..])
            .map_err(|e| BasescopeError::Serialization(e.to_string()))?;
        ChunkData::try_from(col).map_err(BasescopeError::Serialization)
    }

    pub fn save_chunk(&self, chunk: &ChunkData) -> Result<(), BasescopeError> {
        ...
        let compressed = zstd::bulk::compress(&out, ZSTD_LEVEL)
            .map_err(|e| BasescopeError::Serialization(format!("zstd compress: {e}")))?;
