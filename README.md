...
- **Live-updating charts** — tx count, gas usage, and tx size per block; base fee / gas / bytes over time; and distribution histograms update as blocks stream in
- **Chart type switching** — press `t` to cycle between tx count, gas used, and tx size (RLP-encoded bytes) views across all charts
